...
namespace h3ga {
	...
	namespace {
		...
		projectFuncfloat g_projectFuncTable_float[16] = {
			NULL, 
				projectE1ontoG2, 
				projectE2ontoG2, 
				projectE12ontoG3, 
	...
	factors: array of points to arrays of float [4][4]
	...
	*/
	...
	int k,
		...
		// pseudoscalar?
		if (k == N) {
			factors[0][1] = factors[0][2] = factors[0][3] = 			
				factors[1][0] = factors[1][2] = factors[1][3] = 			
				factors[2][0] = factors[2][1] = factors[2][3] = 			
				factors[3][0] = factors[3][1] = factors[3][2] = 			
				0.0f;
			factors[0][0] = factors[1][1] = factors[2][2] = factors[3][3] =  1.0f;
		...
		if (ortho) { // do a gram-schmidt of the factors
			for (int i = 0; i < k; i++) {
				...
				for (int j = 0; j < i; j++) {
					...
					float s = Fi[0] * Fj[0] + Fi[1] * Fj[1] + Fi[2] * Fj[2] + Fi[3] * Fj[3] ;
				...
				float n = sqrtf(Fi[0] * Fi[0] + Fi[1] * Fi[1] + Fi[2] * Fi[2] + Fi[3] * Fi[3] );
				if (n != 0.0f) {
					Fi[0] /= n;
					Fi[1] /= n;
					Fi[2] /= n;
	...
	mv::Float factorizeBlade(const mv &X, point factor[4], int gradeOfX /* = -1 */) {
		...
		if (k < 0) {
			...
			if (k > 4) return 0.0; // invalid input
		}

		// collect arguments to fastFactor()
		const mv::Float *c = X.m_c;
		mv::Float *factors[4] = {factor[0].m_c, factor[1].m_c , factor[2].m_c , factor[3].m_c };
		mv::Float scale;
	...
	namespace {
		float popEG1(const float *A, const float *B, float *R) {
			...
			R[3] =  + -1.0f * A[3] * B[0] + 1.0f * A[0] * B[3];
			R[4] =  + -1.0f * A[3] * B[1] + 1.0f * A[1] * B[3];
			R[5] =  + -1.0f * A[3] * B[2] + 1.0f * A[2] * B[3];
		...
		float popEG2(const float *A, const float *B, float *R) {
			...
			R[1] =  + 1.0f * A[4] * B[0] + -1.0f * A[3] * B[1] + 1.0f * A[0] * B[3];
			R[2] =  + 1.0f * A[5] * B[1] + -1.0f * A[4] * B[2] + 1.0f * A[1] * B[3];
			R[3] =  + -1.0f * A[5] * B[0] + 1.0f * A[3] * B[2] + 1.0f * A[2] * B[3];
			return B[0] * B[0] + B[1] * B[1] + B[2] * B[2] + B[3] * B[3];
		}
		float popE1G2(const float *A, const float *B, float *R) {
			R[0] =  + 1.0f * A[2] * B[0] + -1.0f * A[0] * B[2];
			R[1] =  + -1.0f * A[3] * B[0] + 1.0f * A[0] * B[3];
			R[2] =  + 1.0f * A[5] * B[0] + 1.0f * A[4] * B[2] + 1.0f * A[1] * B[3];
			R[3] =  + -1.0f * A[3] * B[2] + 1.0f * A[2] * B[3];
			return B[0] * B[0] + B[2] * B[2] + B[3] * B[3];
		}
		float popE2G2(const float *A, const float *B, float *R) {
			R[0] =  + -1.0f * A[1] * B[0] + 1.0f * A[0] * B[1];
			R[1] =  + -1.0f * A[4] * B[0] + 1.0f * A[0] * B[4];
			R[2] =  + -1.0f * A[4] * B[1] + 1.0f * A[1] * B[4];
			R[3] =  + 1.0f * A[5] * B[0] + 1.0f * A[3] * B[1] + 1.0f * A[2] * B[4];
			return B[0] * B[0] + B[1] * B[1] + B[4] * B[4];
		}
		float popE3G2(const float *A, const float *B, float *R) {
			R[0] =  + -1.0f * A[2] * B[1] + 1.0f * A[1] * B[2];
			R[1] =  + 1.0f * A[3] * B[1] + 1.0f * A[4] * B[2] + 1.0f * A[0] * B[5];
			R[2] =  + -1.0f * A[5] * B[1] + 1.0f * A[1] * B[5];
			R[3] =  + -1.0f * A[5] * B[2] + 1.0f * A[2] * B[5];
			return B[1] * B[1] + B[2] * B[2] + B[5] * B[5];
		}
		float popE4G2(const float *A, const float *B, float *R) {
			R[0] =  + -1.0f * A[1] * B[3] + -1.0f * A[2] * B[4] + -1.0f * A[0] * B[5];
			R[1] =  + -1.0f * A[4] * B[3] + 1.0f * A[3] * B[4];
			R[2] =  + -1.0f * A[5] * B[4] + 1.0f * A[4] * B[5];
			R[3] =  + 1.0f * A[5] * B[3] + -1.0f * A[3] * B[5];
			return B[3] * B[3] + B[4] * B[4] + B[5] * B[5];
		}
		float popEG3(const float *A, const float *B, float *R) {
			R[0] =  + -1.0f * A[2] * B[0] + -1.0f * A[3] * B[1] + -1.0f * A[1] * B[2] + 1.0f * A[0] * B[3];
			return B[0] * B[0] + B[1] * B[1] + B[2] * B[2] + B[3] * B[3];
		}
		float popE1G3(const float *A, const float *B, float *R) {
			R[0] =  + -1.0f * A[3] * B[0] + 1.0f * A[1] * B[2] + 1.0f * A[0] * B[3];
			return B[0] * B[0] + B[2] * B[2] + B[3] * B[3];
		}
		float popE2G3(const float *A, const float *B, float *R) {
			R[0] =  + 1.0f * A[2] * B[0] + -1.0f * A[1] * B[1] + 1.0f * A[0] * B[4];
			return B[0] * B[0] + B[1] * B[1] + B[4] * B[4];
		}
		float popE3G3(const float *A, const float *B, float *R) {
			R[0] =  + 1.0f * A[3] * B[1] + -1.0f * A[2] * B[2] + 1.0f * A[0] * B[5];
			return B[1] * B[1] + B[2] * B[2] + B[5] * B[5];
		}
		float popE4G3(const float *A, const float *B, float *R) {
			R[0] =  + 1.0f * A[2] * B[3] + 1.0f * A[3] * B[4] + 1.0f * A[1] * B[5];
			return B[3] * B[3] + B[4] * B[4] + B[5] * B[5];
		}
		...
		popFunc g_popFuncTable_float[4][16] = {
			{ // grade 0
				NULL, 
				NULL, 
					NULL, 
		...
		void meetFromJoin_A1_B1_J1(const float *A, const float *B, const float *J, float *M) {
			M[0] = 1.0f * A[0] * B[0] * J[0] + 1.0f * A[0] * B[1] * J[1] + 1.0f * A[0] * B[2] * J[2] + 1.0f * A[0] * B[3] * J[3];
			M[1] = 1.0f * A[1] * B[0] * J[0] + 1.0f * A[1] * B[1] * J[1] + 1.0f * A[1] * B[2] * J[2] + 1.0f * A[1] * B[3] * J[3];
			M[2] = 1.0f * A[2] * B[0] * J[0] + 1.0f * A[2] * B[1] * J[1] + 1.0f * A[2] * B[2] * J[2] + 1.0f * A[2] * B[3] * J[3];
			M[3] = 1.0f * A[3] * B[0] * J[0] + 1.0f * A[3] * B[1] * J[1] + 1.0f * A[3] * B[2] * J[2] + 1.0f * A[3] * B[3] * J[3];
		}
		void meetFromJoin_A1_B1_J2(const float *A, const float *B, const float *J, float *M) {
			M[0] = 1.0f * A[0] * B[1] * J[0] + -1.0f * A[0] * B[2] * J[2] + 1.0f * A[0] * B[3] * J[3] + -1.0f * A[1] * B[0] * J[0] + 1.0f * A[1] * B[2] * J[1] + 1.0f * A[1] * B[3] * J[4] + 1.0f * A[2] * B[0] * J[2] + -1.0f * A[2] * B[1] * J[1] + 1.0f * A[2] * B[3] * J[5] + -1.0f * A[3] * B[0] * J[3] + -1.0f * A[3] * B[1] * J[4] + -1.0f * A[3] * B[2] * J[5];
		}
		void meetFromJoin_A2_B1_J1(const float *A, const float *B, const float *J, float *M) {
			...
			M[2] = 1.0f * A[2] * B[0] * J[0] + 1.0f * A[2] * B[1] * J[1] + 1.0f * A[2] * B[2] * J[2] + 1.0f * A[2] * B[3] * J[3];
			M[3] = 1.0f * A[3] * B[0] * J[0] + 1.0f * A[3] * B[1] * J[1] + 1.0f * A[3] * B[2] * J[2] + 1.0f * A[3] * B[3] * J[3];
			M[4] = 1.0f * A[4] * B[0] * J[0] + 1.0f * A[4] * B[1] * J[1] + 1.0f * A[4] * B[2] * J[2] + 1.0f * A[4] * B[3] * J[3];
			M[5] = 1.0f * A[5] * B[0] * J[0] + 1.0f * A[5] * B[1] * J[1] + 1.0f * A[5] * B[2] * J[2] + 1.0f * A[5] * B[3] * J[3];
		}
		void meetFromJoin_A2_B1_J2(const float *A, const float *B, const float *J, float *M) {
			M[0] = 1.0f * A[0] * B[0] * J[0] + -1.0f * A[0] * B[2] * J[1] + -1.0f * A[0] * B[3] * J[4] + 1.0f * A[2] * B[0] * J[2] + -1.0f * A[2] * B[1] * J[1] + 1.0f * A[2] * B[3] * J[5] + 1.0f * A[3] * B[0] * J[3] + 1.0f * A[3] * B[1] * J[4] + 1.0f * A[3] * B[2] * J[5];
			M[1] = 1.0f * A[0] * B[1] * J[0] + -1.0f * A[0] * B[2] * J[2] + 1.0f * A[0] * B[3] * J[3] + -1.0f * A[1] * B[0] * J[2] + 1.0f * A[1] * B[1] * J[1] + -1.0f * A[1] * B[3] * J[5] + 1.0f * A[4] * B[0] * J[3] + 1.0f * A[4] * B[1] * J[4] + 1.0f * A[4] * B[2] * J[5];
			M[2] = -1.0f * A[1] * B[0] * J[0] + 1.0f * A[1] * B[2] * J[1] + 1.0f * A[1] * B[3] * J[4] + -1.0f * A[2] * B[1] * J[0] + 1.0f * A[2] * B[2] * J[2] + -1.0f * A[2] * B[3] * J[3] + 1.0f * A[5] * B[0] * J[3] + 1.0f * A[5] * B[1] * J[4] + 1.0f * A[5] * B[2] * J[5];
			M[3] = 1.0f * A[3] * B[1] * J[0] + -1.0f * A[3] * B[2] * J[2] + 1.0f * A[3] * B[3] * J[3] + -1.0f * A[4] * B[0] * J[0] + 1.0f * A[4] * B[2] * J[1] + 1.0f * A[4] * B[3] * J[4] + 1.0f * A[5] * B[0] * J[2] + -1.0f * A[5] * B[1] * J[1] + 1.0f * A[5] * B[3] * J[5];
		}
		void meetFromJoin_A2_B1_J3(const float *A, const float *B, const float *J, float *M) {
			M[0] = 1.0f * A[0] * B[2] * J[0] + 1.0f * A[0] * B[3] * J[1] + 1.0f * A[1] * B[0] * J[0] + 1.0f * A[1] * B[3] * J[2] + 1.0f * A[2] * B[1] * J[0] + 1.0f * A[2] * B[3] * J[3] + -1.0f * A[3] * B[1] * J[1] + 1.0f * A[3] * B[2] * J[3] + 1.0f * A[4] * B[0] * J[1] + -1.0f * A[4] * B[2] * J[2] + -1.0f * A[5] * B[0] * J[3] + 1.0f * A[5] * B[1] * J[2];
		}
		void meetFromJoin_A2_B2_J2(const float *A, const float *B, const float *J, float *M) {
			M[0] = 1.0f * A[0] * B[0] * J[0] + 1.0f * A[0] * B[1] * J[1] + 1.0f * A[0] * B[2] * J[2] + 1.0f * A[0] * B[3] * J[3] + 1.0f * A[0] * B[4] * J[4] + 1.0f * A[0] * B[5] * J[5];
			M[1] = 1.0f * A[1] * B[0] * J[0] + 1.0f * A[1] * B[1] * J[1] + 1.0f * A[1] * B[2] * J[2] + 1.0f * A[1] * B[3] * J[3] + 1.0f * A[1] * B[4] * J[4] + 1.0f * A[1] * B[5] * J[5];
			M[2] = 1.0f * A[2] * B[0] * J[0] + 1.0f * A[2] * B[1] * J[1] + 1.0f * A[2] * B[2] * J[2] + 1.0f * A[2] * B[3] * J[3] + 1.0f * A[2] * B[4] * J[4] + 1.0f * A[2] * B[5] * J[5];
			M[3] = 1.0f * A[3] * B[0] * J[0] + 1.0f * A[3] * B[1] * J[1] + 1.0f * A[3] * B[2] * J[2] + 1.0f * A[3] * B[3] * J[3] + 1.0f * A[3] * B[4] * J[4] + 1.0f * A[3] * B[5] * J[5];
			M[4] = 1.0f * A[4] * B[0] * J[0] + 1.0f * A[4] * B[1] * J[1] + 1.0f * A[4] * B[2] * J[2] + 1.0f * A[4] * B[3] * J[3] + 1.0f * A[4] * B[4] * J[4] + 1.0f * A[4] * B[5] * J[5];
			M[5] = 1.0f * A[5] * B[0] * J[0] + 1.0f * A[5] * B[1] * J[1] + 1.0f * A[5] * B[2] * J[2] + 1.0f * A[5] * B[3] * J[3] + 1.0f * A[5] * B[4] * J[4] + 1.0f * A[5] * B[5] * J[5];
		}
		void meetFromJoin_A2_B2_J3(const float *A, const float *B, const float *J, float *M) {
			M[0] = -1.0f * A[0] * B[2] * J[0] + 1.0f * A[0] * B[3] * J[1] + -1.0f * A[0] * B[5] * J[2] + 1.0f * A[2] * B[0] * J[0] + 1.0f * A[2] * B[3] * J[3] + -1.0f * A[2] * B[4] * J[2] + -1.0f * A[3] * B[0] * J[1] + -1.0f * A[3] * B[1] * J[2] + -1.0f * A[3] * B[2] * J[3];
			M[1] = 1.0f * A[0] * B[1] * J[0] + 1.0f * A[0] * B[4] * J[1] + -1.0f * A[0] * B[5] * J[3] + -1.0f * A[1] * B[0] * J[0] + -1.0f * A[1] * B[3] * J[3] + 1.0f * A[1] * B[4] * J[2] + -1.0f * A[4] * B[0] * J[1] + -1.0f * A[4] * B[1] * J[2] + -1.0f * A[4] * B[2] * J[3];
			M[2] = 1.0f * A[1] * B[2] * J[0] + -1.0f * A[1] * B[3] * J[1] + 1.0f * A[1] * B[5] * J[2] + -1.0f * A[2] * B[1] * J[0] + -1.0f * A[2] * B[4] * J[1] + 1.0f * A[2] * B[5] * J[3] + -1.0f * A[5] * B[0] * J[1] + -1.0f * A[5] * B[1] * J[2] + -1.0f * A[5] * B[2] * J[3];
			M[3] = 1.0f * A[3] * B[1] * J[0] + 1.0f * A[3] * B[4] * J[1] + -1.0f * A[3] * B[5] * J[3] + 1.0f * A[4] * B[2] * J[0] + -1.0f * A[4] * B[3] * J[1] + 1.0f * A[4] * B[5] * J[2] + 1.0f * A[5] * B[0] * J[0] + 1.0f * A[5] * B[3] * J[3] + -1.0f * A[5] * B[4] * J[2];
		}
		void meetFromJoin_A2_B2_J4(const float *A, const float *B, const float *J, float *M) {
			M[0] = 1.0f * A[0] * B[5] * J[0] + 1.0f * A[1] * B[3] * J[0] + 1.0f * A[2] * B[4] * J[0] + 1.0f * A[3] * B[1] * J[0] + 1.0f * A[4] * B[2] * J[0] + 1.0f * A[5] * B[0] * J[0];
		}
		...
		void meetFromJoin_A3_B1_J2(const float *A, const float *B, const float *J, float *M) {
			M[0] = 1.0f * A[0] * B[0] * J[2] + -1.0f * A[0] * B[1] * J[1] + 1.0f * A[0] * B[3] * J[5] + -1.0f * A[1] * B[0] * J[3] + -1.0f * A[1] * B[1] * J[4] + -1.0f * A[1] * B[2] * J[5];
			M[1] = 1.0f * A[0] * B[1] * J[0] + -1.0f * A[0] * B[2] * J[2] + 1.0f * A[0] * B[3] * J[3] + -1.0f * A[2] * B[0] * J[3] + -1.0f * A[2] * B[1] * J[4] + -1.0f * A[2] * B[2] * J[5];
			M[2] = -1.0f * A[0] * B[0] * J[0] + 1.0f * A[0] * B[2] * J[1] + 1.0f * A[0] * B[3] * J[4] + -1.0f * A[3] * B[0] * J[3] + -1.0f * A[3] * B[1] * J[4] + -1.0f * A[3] * B[2] * J[5];
			M[3] = 1.0f * A[1] * B[0] * J[0] + -1.0f * A[1] * B[2] * J[1] + -1.0f * A[1] * B[3] * J[4] + 1.0f * A[3] * B[0] * J[2] + -1.0f * A[3] * B[1] * J[1] + 1.0f * A[3] * B[3] * J[5];
			M[4] = 1.0f * A[1] * B[1] * J[0] + -1.0f * A[1] * B[2] * J[2] + 1.0f * A[1] * B[3] * J[3] + -1.0f * A[2] * B[0] * J[2] + 1.0f * A[2] * B[1] * J[1] + -1.0f * A[2] * B[3] * J[5];
			M[5] = -1.0f * A[2] * B[0] * J[0] + 1.0f * A[2] * B[2] * J[1] + 1.0f * A[2] * B[3] * J[4] + -1.0f * A[3] * B[1] * J[0] + 1.0f * A[3] * B[2] * J[2] + -1.0f * A[3] * B[3] * J[3];
		}
		void meetFromJoin_A3_B1_J3(const float *A, const float *B, const float *J, float *M) {
			M[0] = 1.0f * A[0] * B[0] * J[0] + 1.0f * A[0] * B[3] * J[2] + 1.0f * A[1] * B[0] * J[1] + -1.0f * A[1] * B[2] * J[2] + 1.0f * A[3] * B[0] * J[3] + -1.0f * A[3] * B[1] * J[2];
			M[1] = 1.0f * A[0] * B[1] * J[0] + 1.0f * A[0] * B[3] * J[3] + 1.0f * A[1] * B[1] * J[1] + -1.0f * A[1] * B[2] * J[3] + -1.0f * A[2] * B[0] * J[3] + 1.0f * A[2] * B[1] * J[2];
			M[2] = 1.0f * A[0] * B[2] * J[0] + 1.0f * A[0] * B[3] * J[1] + -1.0f * A[2] * B[0] * J[1] + 1.0f * A[2] * B[2] * J[2] + -1.0f * A[3] * B[1] * J[1] + 1.0f * A[3] * B[2] * J[3];
			M[3] = 1.0f * A[1] * B[2] * J[0] + 1.0f * A[1] * B[3] * J[1] + 1.0f * A[2] * B[0] * J[0] + 1.0f * A[2] * B[3] * J[2] + 1.0f * A[3] * B[1] * J[0] + 1.0f * A[3] * B[3] * J[3];
		}
		void meetFromJoin_A3_B1_J4(const float *A, const float *B, const float *J, float *M) {
			M[0] = 1.0f * A[0] * B[3] * J[0] + -1.0f * A[1] * B[2] * J[0] + -1.0f * A[2] * B[0] * J[0] + -1.0f * A[3] * B[1] * J[0];
		}
		...
		void meetFromJoin_A3_B2_J3(const float *A, const float *B, const float *J, float *M) {
			M[0] = 1.0f * A[0] * B[0] * J[0] + 1.0f * A[0] * B[3] * J[3] + -1.0f * A[0] * B[4] * J[2] + 1.0f * A[1] * B[0] * J[1] + 1.0f * A[1] * B[1] * J[2] + 1.0f * A[1] * B[2] * J[3];
			M[1] = 1.0f * A[0] * B[1] * J[0] + 1.0f * A[0] * B[4] * J[1] + -1.0f * A[0] * B[5] * J[3] + 1.0f * A[2] * B[0] * J[1] + 1.0f * A[2] * B[1] * J[2] + 1.0f * A[2] * B[2] * J[3];
			M[2] = 1.0f * A[0] * B[2] * J[0] + -1.0f * A[0] * B[3] * J[1] + 1.0f * A[0] * B[5] * J[2] + 1.0f * A[3] * B[0] * J[1] + 1.0f * A[3] * B[1] * J[2] + 1.0f * A[3] * B[2] * J[3];
			M[3] = -1.0f * A[1] * B[2] * J[0] + 1.0f * A[1] * B[3] * J[1] + -1.0f * A[1] * B[5] * J[2] + 1.0f * A[3] * B[0] * J[0] + 1.0f * A[3] * B[3] * J[3] + -1.0f * A[3] * B[4] * J[2];
			M[4] = 1.0f * A[1] * B[1] * J[0] + 1.0f * A[1] * B[4] * J[1] + -1.0f * A[1] * B[5] * J[3] + -1.0f * A[2] * B[0] * J[0] + -1.0f * A[2] * B[3] * J[3] + 1.0f * A[2] * B[4] * J[2];
			M[5] = 1.0f * A[2] * B[2] * J[0] + -1.0f * A[2] * B[3] * J[1] + 1.0f * A[2] * B[5] * J[2] + -1.0f * A[3] * B[1] * J[0] + -1.0f * A[3] * B[4] * J[1] + 1.0f * A[3] * B[5] * J[3];
		}
		void meetFromJoin_A3_B2_J4(const float *A, const float *B, const float *J, float *M) {
			M[0] = 1.0f * A[0] * B[3] * J[0] + 1.0f * A[1] * B[2] * J[0] + -1.0f * A[3] * B[0] * J[0];
			M[1] = 1.0f * A[0] * B[4] * J[0] + -1.0f * A[1] * B[1] * J[0] + 1.0f * A[2] * B[0] * J[0];
			M[2] = 1.0f * A[0] * B[5] * J[0] + -1.0f * A[2] * B[2] * J[0] + 1.0f * A[3] * B[1] * J[0];
			M[3] = 1.0f * A[1] * B[5] * J[0] + 1.0f * A[2] * B[3] * J[0] + 1.0f * A[3] * B[4] * J[0];
		}
		...
		void meetFromJoin_A3_B3_J4(const float *A, const float *B, const float *J, float *M) {
			...
			M[3] = -1.0f * A[1] * B[3] * J[0] + 1.0f * A[3] * B[1] * J[0];
			M[4] = 1.0f * A[1] * B[2] * J[0] + -1.0f * A[2] * B[1] * J[0];
			M[5] = 1.0f * A[2] * B[3] * J[0] + -1.0f * A[3] * B[2] * J[0];
		}
		typedef void (*meetFromJoinFunc_float)(const float*, const float*, const float*, float*);
		meetFromJoinFunc_float meetFromJoinTable_float[5][5][5]  = {
			{
			...
			{
				{NULL,NULL,NULL,NULL,NULL},
				{NULL,meetFromJoin_A1_B1_J1,meetFromJoin_A1_B1_J2,NULL,NULL},
				{NULL,NULL,NULL,NULL,NULL},
				{NULL,NULL,NULL,NULL,NULL},
				{NULL,NULL,NULL,NULL,NULL}
			},
			{
				{NULL,NULL,NULL,NULL,NULL},
				{NULL,meetFromJoin_A2_B1_J1,meetFromJoin_A2_B1_J2,meetFromJoin_A2_B1_J3,NULL},
				{NULL,NULL,meetFromJoin_A2_B2_J2,meetFromJoin_A2_B2_J3,meetFromJoin_A2_B2_J4},
				{NULL,NULL,NULL,NULL,NULL},
				{NULL,NULL,NULL,NULL,NULL}
			},
			{
				{NULL,NULL,NULL,NULL,NULL},
				{NULL,meetFromJoin_A3_B1_J1,meetFromJoin_A3_B1_J2,meetFromJoin_A3_B1_J3,meetFromJoin_A3_B1_J4},
				{NULL,NULL,meetFromJoin_A3_B2_J2,meetFromJoin_A3_B2_J3,meetFromJoin_A3_B2_J4},
				{NULL,NULL,NULL,meetFromJoin_A3_B3_J3,meetFromJoin_A3_B3_J4},
				{NULL,NULL,NULL,NULL,NULL}
			},
			{
				{NULL,NULL,NULL,NULL,NULL},
	...
	Input:
	gradeA, gradeB: grade of A and B range [0 ... 4]
	...
	*/
	...
	int gradeA, const float *coordA, 
		...
		else if (gradeA == 4) { // A is pseudoscalar?
		...
		else if (gradeB == 4) { // B is pseudoscalar?
		...
		{ // project basis vectors bvM onto B, wedge to A
			...
			int i = 0;
			float tmp1[16]; // an array of n choose floor(n/2) would be large enough
			float *Jtmp = tmp1;
			for (int i = 1; i <= M; i <<= 1) {
				if ((M & i) == 0) continue;
				...
				if (n2 > EPS * EPS) {
					...
					if (gradeJ == 4) break; // terminate when J is pseudoscalar
				}
			}
		}
