...
namespace e2ga {
	...
	namespace {
		...
		projectFuncfloat g_projectFuncTable_float[4] = {
			NULL, 
				NULL, 
				NULL, 
				NULL
	...
	factors: array of points to arrays of float [2][2]
	...
	*/
	...
	int k,
		...
		if (ortho) { // do a gram-schmidt of the factors
			for (int i = 0; i < k; i++) {
				...
				float n = sqrtf(Fi[0] * Fi[0] + Fi[1] * Fi[1] );
				if (n != 0.0f) {
					Fi[0] /= n;
					Fi[1] /= n;
				}
	...
	mv::Float fastFactorBlade(const mv &X, vector factor[2], int gradeOfX /* = -1 */) {
		int k = gradeOfX;
		if (k < 0) {
			// only for single grade usage: find first non-zero grade part:
			k = lowestOneBit(X.m_gu);
			if (k > 2) return 0.0; // invalid input
		}

		// collect arguments to fastFactor()
		const mv::Float *c = X.m_c;
		mv::Float *factors[2] = {factor[0].m_c, factor[1].m_c };
		mv::Float scale;
	...
	namespace {
		...
		popFunc g_popFuncTable_float[2][4] = {
			{ // grade 0
				NULL, 
				NULL, 
					NULL, 
		...
		void meetFromJoin_A1_B1_J1(const float *A, const float *B, const float *J, float *M) {
			M[0] = 1.0f * A[0] * B[0] * J[0] + 1.0f * A[0] * B[1] * J[1];
			M[1] = 1.0f * A[1] * B[0] * J[0] + 1.0f * A[1] * B[1] * J[1];
		}
		...
		meetFromJoinFunc_float meetFromJoinTable_float[3][3][3]  = {
			{
				{NULL,NULL,NULL},
				{NULL,NULL,NULL},
				{NULL,NULL,NULL}
			...
			{
				{NULL,NULL,NULL},
				{NULL,meetFromJoin_A1_B1_J1,meetFromJoin_A1_B1_J2},
				{NULL,NULL,NULL}
			},
			{
				{NULL,NULL,NULL},
	...
	namespace {
		...
		inline void copyBlade(int gradeSrc, const float *coordSrc, int &gradeDst, float *coordDst) {
			...
			if ((gradeSrc == 0) || (gradeSrc == 2)) return;
	...
	Input:
	gradeA, gradeB: grade of A and B range [0 ... 2]
	...
	*/
	...
	int gradeA, const float *coordA, 
		...
		else if (gradeA == 2) { // A is pseudoscalar?
		...
		else if (gradeB == 2) { // B is pseudoscalar?
		...
		{ // project basis vectors bvM onto B, wedge to A
			...
			int i = 0;
			float tmp1[4]; // an array of n choose floor(n/2) would be large enough
			float *Jtmp = tmp1;
			for (int i = 1; i <= M; i <<= 1) {
				if ((M & i) == 0) continue;
				...
				if (n2 > EPS * EPS) {
					...
					if (gradeJ == 2) break; // terminate when J is pseudoscalar
				}
			}
		}
