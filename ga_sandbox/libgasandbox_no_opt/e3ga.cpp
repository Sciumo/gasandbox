...
#include "e3ga.h"
	...
	namespace g2Profiling {
		...
		void save(const char *filename /*= "E:\\ga\\ga_sandbox\\ga_sandbox\\libgasandbox_no_opt\\e3ga.gp2"*/, bool append /*= false*/) {
		}
	...
	mv gp(const mv& x, const mv& y) {
		...
		if (((x.m_gu & 4) != 0)) {
			...
			if (((y.m_gu & 2) != 0)) {
				...
				__tmp_coord_array_2__[7] += ((__x_xpd__[2][1] * __y_xpd__[1][0]) + (__x_xpd__[2][0] * __y_xpd__[1][2]) + (__x_xpd__[2][2] * __y_xpd__[1][1]));
	...
	mv apply_om(const om& x, const mv& y) {
		...
		y.expand(__y_xpd__, true);
		if (((y.m_gu & 2) != 0)) {
			__tmp_coord_array_14__[1] += ((x.m_c[1] * __y_xpd__[1][1]) + (x.m_c[2] * __y_xpd__[1][2]) + (x.m_c[0] * __y_xpd__[1][0]));
